    ...
    <div class="row justi-cfyontent-center" data-aos="fade-up" data-aos-delay="150">
      <div class="col-xl-12 col-lg-8">
        	<a href="#" class="heading">
        		<h1 class="text-center display-5">Veiw Details</h1>
        	</a>
      </div>
    </div>
  </div>
...
<?php 
	$dis = $_GET['cat_id'];
	$sql = "SELECT * FROM `category_details` WHERE `add_calories` LIKE '%$cat_id%' OR `content_title` LIKE '%$cat_id%' OR `incridients` LIKE '%$cat_id%' OR `type` LIKE '%$cat_id%' OR `fats` LIKE '%$cat_id%' OR `cholesterol` LIKE '%$cat_id%' OR `sodium` LIKE '%$cat_id%' OR `potassium` LIKE '%$cat_id%' OR `carbohydrate` LIKE '%$cat_id%' OR `protein` LIKE '%$cat_id%'";
  $run = $con->query($sql);	
	    	...
	    	<table class="table text-center table-bordered">
				  ...
				  <tbody >
				    ...
				    <tr>
				      ...
				      <td scope="col" style="text-transform: lowercase!important;"><?php echo $fetch['add_calories']; ?> g</td>
				    </tr>
				  </tbody>
				</table>
	  	</article>
